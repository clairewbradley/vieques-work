(function () {
	...
	var map = L.map('map', {
		zoomControl: false,
	}).setView([18.153965, -65.445150], 12);
	...
	// add 1937 tile, or the first in line if it does not exist
	if (baseLayers['CartoCollectiveAzure.1937']) {
		baseLayers['CartoCollectiveAzure.1937'].addTo(map);
	 } else {
		baseLayers[Object.keys(baseLayers)[1]].addTo(map);
	}

	// if a layer is selected and if it has bounds and the bounds are not in the
	...
	map.addControl(new (L.Control.extend({
		...
		onAdd: function (map) {
			...
			L.DomUtil.create('h4', null, container).innerHTML = '';
			var providerNames = L.DomUtil.create('code', 'provider-names');
